...
server.registerTool(
  'summarize_speeches',
  {
    ...
    inputSchema: z.object({
      ...
      focus: z.string().optional().describe('要約の焦点（例: "生成AI規制", "財政政策"）。省略可'),
    }),
  },
...
server.registerTool(
  'summarize_meeting',
  {
    ...
    inputSchema: z.object({
      ...
      focus: z.string().optional().describe('要約の焦点（例: "生成AI規制"）。省略可'),
    }),
  },
  ...
  'compare_over_time',
  {
    ...
    inputSchema: z.object({
      ...
      mode: z
        ...
        .default('standard')
        .describe(
          '出力粒度: brief（主要変化2〜3件）/ standard（標準）/ detailed（詳細・コスト高）',
        ),
      include_topics: z.boolean().default(true).describe('論点比較を含めるか'),
      include_speaker_changes: z.boolean().default(true).describe('発言者傾向の比較を含めるか'),
  ...
  'summarize_qa_pairs',
  {
    ...
    inputSchema: z.object({
      ...
      mode: z
        ...
        .default('standard')
        .describe(
          '出力粒度: brief（3〜5件・短縮）/ standard（5〜10件）/ detailed（詳細・コスト高）',
        ),
      max_pairs: z
        .number()
        .int()
      ...
      include_unanswered: z.boolean().default(true).describe('明確な答弁が取れない質問も含めるか'),
    }),
  },
  ...
  'compare_by_party',
  {
    ...
    inputSchema: z.object({
      ...
      mode: z
        ...
        .default('standard')
        .describe(
          '出力粒度: brief（主要政党のみ・簡潔）/ standard（標準）/ detailed（詳細・コスト高）',
        ),
  ...
  'analyze_topic_changes',
  {
    ...
    inputSchema: z.object({
      ...
      max_items_per_period: z
        ...
        .default(20)
        .describe('各期間で取得する最大発言件数（既定: 20）'),
      include_emerging_topics: z.boolean().default(true).describe('新規論点（new）を含めるか'),
